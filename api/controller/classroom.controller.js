...
const create = (req, res) => {
  ...
  if (Object.keys(message).length > 0) {
    ...
    var sql =
      'INSERT INTO classroom (`course_id`,`teacher_id`,`course_price`,`course_gendertion`,`time_start`, `time_end`, `date_start`,`date_end`,`duration_h`) VALUES (?,?,?,?,?,?,?,?,?)';
    db.query(
      sql,
      [course_id, teacher_id, course_price, course_gendertion,time_start,time_end,date_start,date_end,duration_h],
      (err, result) => {
        if (!err) {
