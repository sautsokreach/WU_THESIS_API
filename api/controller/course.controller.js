...
// permision_course_view
// permision_course_create
// permision_course_edit
// permision_course_remove
...
const getAll = async (req, res) => {
    ...
    var offset = Number(body.page - 1) * 10;
    var sql = "SELECT "+
        " co.course_id,"+ 
        " co.name,"+ 
        " co.full_price,"+
    ...
    sql += " ORDER BY co.course_id DESC ";
    sql += " LIMIT 10 OFFSET "+offset;
    db.query(sql, (err, result) => {
        if (!res.err) {
            var sqlCat = "SELECT * FROM category";
            db.query(sqlCat,(err,cat)=>{
                category = cat 
                res.json({
                    list: result,
                    category : cat,
